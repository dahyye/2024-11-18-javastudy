package com.sist.exception;
/*
 	예외처리 간접처리 : throws
 		=> 예외회피
 		=> 예상되는 예외를 선언 => 이런 예외 발생할 것 같다
 							 --------------------
 							 컴파일러에 알려준다
 		=> 이 메소드를 사용시에는 반드시 예외처리 후 사용이 가능
 			=> throws / try ~ catch 를 이용할 수 있다
 		=> 사용자 정의는 사용빈도가 적다 / 라이브러리가 많이 존재
 			---------------------
 			코딩 소스가 많은데 중간에 예외처리할 문장이 있는 경우
 			
 			
 			형식) 
 			public void display() throws 예외처리클래스  => 다음사람이 쓸 때 예외처리하고써라 -> 떠넘기기
 			  => 여러개일 경우 , 로 구문
 			  => 순서가 없다
			  => 무조건 쓰는게 아니라 예상되는 예외를 지정
 	
 */
public class 예외처리_1 {

//	public int div(int a, int b) throws ArithmeticException
//	{
//		return a/b;
//	}
//	
//	public void print()
//	{
//		int c = div(10,0);
//		
//	}
	public void display() throws Exception, ClassNotFoundException
	{
		System.out.println("display call...");
	}
	//CheckException이라서 호출 시 반드시 예외처리 해야함
	//UnCheckException 은 throws 문장을 사용하지 않는데
	
	public void print()
	{
		try {
			display();
		} catch (ClassNotFoundException e) {
			// TODO: handle exception
		}catch (Exception e) {
			// TODO: handle exception
		}
		
	}
	
	public void print2()
	{
		try {
			display();
		} catch (Exception e) {
			// TODO: handle exception
		}
		
	}
	public void print3() throws Exception, ClassNotFoundException
	{
		display();
		
	}
	public void print4() throws Exception
	{
		display();
		
	}
	/*
	 	소스가 없는 경우 => 새로운 메소드를 만드는 경우
	 		try ~ catch => 개발
	 	
	 	이미 소스 코딩이 있는 경우 => 새로운 기능 추가
	  		throws => 유지보수
	  		------
	  		1. 예상이 되는 예외를 메소드뒤에 선언
	  		2. 메소드 호출에 반ㄷ시 예외처리 후 호출
	  		-------------------------------
	  		 => 직접처리****
	  		 => 선언하고 사용할 수 있따
	  
	 */
	public static void main(String[] args) {
		// TODO Auto-generated method stub
		try {
			Thread.sleep(1000);
		} catch (InterruptedException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	}

}
